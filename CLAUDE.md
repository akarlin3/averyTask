...
**v1.7 — PrismTask-Timeline-Class scope (in-flight, May 2026):** extends `ProjectEntity` so PrismTask itself can be managed as a project inside PrismTask. Audit: `docs/audits/PRISMTASK_TIMELINE_CLASS_AUDIT.md`. Single mega-PR per operator override (`O1`); audit's recommended 4-PR stack and "no UI in this PR" (`O3`) were both overridden — a read-only roadmap view ships with the schema. Capabilities:

- **Project phases**: new `ProjectPhaseEntity` + `ProjectPhaseDao` (CASCADE FK to projects). Per-project ordered list with optional date bounds and a version-anchor string. `tasks.phase_id` (nullable, SET_NULL on phase delete) maps a task to a phase.
- **Risk register**: new `ProjectRiskEntity` + `ProjectRiskDao`. Per-project risks keyed by `RiskLevel` enum (LOW / MEDIUM / HIGH); `resolved_at` non-null marks the risk retired but kept for history.
- **External anchors**: new `ExternalAnchorEntity` storing the variant payload as `anchor_json`. Sealed class `ExternalAnchor` (`CalendarDeadline` / `NumericThreshold` / `BooleanGate`) round-trips through `ExternalAnchorJsonAdapter`, mirroring the polymorphic-by-discriminator pattern in `AutomationJsonAdapter`. Anchor's phase FK is SET_NULL so phase reshuffles don't drop anchors.
- **Task dependencies**: new `TaskDependencyEntity` (directed `blocker_task_id → blocked_task_id` edge). `DependencyCycleGuard` runs a DFS at the write site (mirrors `AutomationEngine.handleEvent` lineage walk); duplicate-edge inserts are no-ops via the unique pair index. Domain layer adds `TaskState.BlockedByDependency` so streak filters drop blocked tasks from BOTH the "met" and "missed" pools (audit § P6).
- **Fractional progress (P9 option a)**: `tasks.progress_percent` (nullable INTEGER 0–100) only used by tasks under a project. Burndown reads `progress_percent ?: (if isCompleted then 100 else 0)`, so the cascade to the 60+ `.isCompleted` callers stays at zero. Backend: alembic `023_add_task_progress_percent.py` + `compute_project_burndown` summation. Web: `utils/projectBurndown.ts` mirror.
- **Sync**: subcollection-from-day-1 strategy (`projects/{p}/phases/{ph}` etc.) per audit § P11. `SyncService` upload/pull/delete/realtime-listener paths cover the 4 new entity types; `taskToMap` / `mapToTask` round-trip `phaseCloudId` / `phaseLocalId`. Phases pull BEFORE tasks so task-phase round-trip resolves on first pass.
- **View-mode UI (`ProjectRoadmapScreen`)**: read-only Compose screen renders phases-with-tasks, the risk register, and external anchors for a single project. Distinct from `TimelineScreen` (the daily time-block view) per the audit's naming-collision flag — phase-Gantt surfaces use the `Roadmap` name, "Timeline" stays reserved for time blocks.
- **Storage**: Room migration 71 → 72 in `Migrations.kt` adds 4 new tables + 2 task columns. Recovery path is the v69→v70 (Automation Engine) precedent: `fallbackToDestructiveMigration` gated by `BuiltInSyncPreferences` if the migration corrupts a tester device.
