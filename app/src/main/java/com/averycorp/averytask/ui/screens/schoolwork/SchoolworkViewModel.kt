package com.averycorp.averytask.ui.screens.schoolwork

import android.content.Context
import android.net.Uri
import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.averycorp.averytask.data.local.entity.AssignmentEntity
import com.averycorp.averytask.data.local.entity.CourseCompletionEntity
...
import com.averycorp.averytask.domain.usecase.ChecklistParser
import com.averycorp.averytask.domain.usecase.ComprehensiveImportResult
import com.averycorp.averytask.domain.usecase.ChecklistParsedTask
...
@HiltViewModel
class SchoolworkViewModel @Inject constructor(
    ...
    val courses: StateFlow<List<CourseEntity>> = repository.getActiveCourses()
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())

    val activeAssignments: StateFlow<List<AssignmentEntity>> = repository.getAllAssignments()
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())

    val todayCompletions: StateFlow<List<CourseCompletionEntity>> = repository.getTodayCompletions()
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())
    ...
    private fun countSubtasks(task: ChecklistParsedTask): Int {
        return task.subtasks.size + task.subtasks.sumOf { countSubtasks(it) }
    }
