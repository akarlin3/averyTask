...
interface ExternalAnchorDao {
    @Query("SELECT * FROM external_anchors WHERE project_id = :projectId ORDER BY id ASC")
    fun observeAnchorsForProject(projectId: Long): Flow<List<ExternalAnchorEntity>>

    @Query("SELECT * FROM external_anchors WHERE phase_id = :phaseId ORDER BY id ASC")
    fun observeAnchorsForPhase(phaseId: Long): Flow<List<ExternalAnchorEntity>>

    @Query("SELECT * FROM external_anchors WHERE project_id = :projectId ORDER BY id ASC")
    suspend fun getAnchorsForProjectOnce(projectId: Long): List<ExternalAnchorEntity>

    @Query("SELECT * FROM external_anchors ORDER BY project_id ASC, id ASC")
    suspend fun getAllAnchorsOnce(): List<ExternalAnchorEntity>
