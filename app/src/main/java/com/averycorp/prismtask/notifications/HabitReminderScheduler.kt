...
/**
 * Schedules every habit-related alarm — daily-time, interval, and
 * follow-up nag suppression. Despite the legacy `MedicationReminderScheduler`
 * name (renamed as part of the v1.4 medication-top-level refactor,
 * spec: `docs/SPEC_MEDICATIONS_TOP_LEVEL.md` §5.1), this class always
 * handled app-wide habit alarms, not just medication habits.
 *
 * Per-medication alarms for the new top-level Medication entity live in
 * [MedicationReminderScheduler] (v1.4+). This class retains the legacy
 * [scheduleAtSpecificTime] / [scheduleSpecificTimes] methods for the
 * transitional `MedicationPreferences.specificTimes` path; those die
 * with the Phase 2 cleanup migration when `MedicationPreferences` is
 * removed.
 */
@Singleton
